...
class Seq2Seq(nn.Module):
    def __init__(self, input_size, output_size, en_embed_size=64, en_hidden_size=128, de_embed_size=64,
        ...
        self.decoder = Decoder(de_embed_size, de_hidden_size, output_size, n_layers, dropout)
    ...
    def forward(self, src, trg, teacher_forcing_ratio=0.5):
        ...
        output = Variable(trg.data[0, :]).to(self.device)  # sos
    ...
    def predict(self, src, max_len=SEQ_LEN):
        ...
        encoder_output, hidden = self.encoder(src)
        hidden = hidden[:self.decoder.n_layers]
        output = Variable(src.data[0, :]).to(self.device)  # so
        predict = []
        for t in range(1, max_len):
            output, hidden, attn_weights = self.decoder(
                output, hidden, encoder_output)
